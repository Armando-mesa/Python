...
for usuario in usuarios:
    ...
    # Crear una versión filtrada eliminando la columna STATEMENT_HASH y duplicados
    if 'STATEMENT_HASH' in df_usuario.columns:
        df_filtrado = df_usuario.drop(columns=['STATEMENT_HASH']).drop_duplicates()
    else:
        df_filtrado = df_usuario.drop_duplicates()
    ...
    with ExcelWriter(ruta_usuario, engine='xlsxwriter') as writer:
        # Guardar las hojas divididas por partes
        ...
        # Guardar la hoja "Archivos Filtrados"
        df_filtrado.to_excel(writer, sheet_name="Archivos Filtrados", index=False)
